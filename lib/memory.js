...
class MemorySystem {
  constructor() {
    this.db = null;
    this.DISTILL_THRESHOLD = 10; // Distill after every 10 conversations
  }
  ...
  async saveConversation(prompt, platform = 'chatgpt') {
    const conversation = {
      ...
      platform,
      timestamp: Date.now(),
      responses: {} // Can be populated later if needed
    };
  ...
  async clearOldConversations(keepCount = 100) {
    ...
    if (conversations.length <= keepCount) return;
...
if (typeof globalThis !== 'undefined') {
  globalThis.MemorySystem = MemorySystem;
}
